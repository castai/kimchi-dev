...
- If a subagent call returns an error of any kind (including protocol violation, timeout, or exit error): do NOT attempt to implement or debug the work yourself. Spawn a replacement subagent with a corrected or simplified prompt. Allow at most one retry per delegated step. If the retry also fails, report the failure clearly and stop.
